...
import com.nps.erm.constants.ErmFieldLabel;
...
@Component(immediate = true, service = ERMFormPDFUtil.class)
public class ERMFormPDFUtil {
	

public  File createFormPDF(long ermId) throws IOException {

		
	File file=null;
		try  {  
			 file=FileUtil.createTempFile("formpdf");
			 ErmInformation ermInformation=  ermInformationLocalService.getErmInformation(ermId);
			 String tokenNo="";
			 String rectificReqMode="";
			 if(ermInformation.getRectificationRequestMode().equalsIgnoreCase(ErmFieldLabel.TOKEN_NO)) {
				 tokenNo=" "+ermInformation.getTokenNo();
				 rectificReqMode="CGMS";
			 } 
			 if(ermInformation.getRectificationRequestMode().equalsIgnoreCase(ErmFieldLabel.Email)) {
				 rectificReqMode="Direct Mail-Letter from sub"; 
			 } 
			 if(ermInformation.getRectificationRequestMode().equalsIgnoreCase("Other")) {
				 rectificReqMode="Req from NPST-PFRDA"; 
			 } 
			   PdfWriter writer = new PdfWriter(file);
			    ...
			    para2.add(text5);
			    Text text6 = new Text("\n c) The subscriber has raised a grievance request in CRA grievance system through "+ermInformation.getRectificationRequestMode()+tokenNo+" dated "+NpstCommonUtil.getDateString(ermInformation.getRectificationDate())+" for rectification of erroneous contribution stating that they mistakenly remitted Rs. "+ermInformation.getRemittedAmount()+" on "+NpstCommonUtil.getDateString(ermInformation.getRemittedDate())+" in Tier I "+ermInformation.getTierType()+". Hence subscriber has requested to refund the amount of Rs. "+ermInformation.getTransferAmount()+" "+ermInformation.getTransferAmount1());
			    para2.add(text6); 
			    if(ermInformation.getTimeType().equalsIgnoreCase("Initial")) {
			   
			    	Text text7 =new Text ("\n d) The subscriber has provided "+ermInformation.getTimeType()+" set of documents on "+NpstCommonUtil.getDateString(ermInformation.getTimeTypeDate())  +" and complete set of documents on "+NpstCommonUtil.getDateString(ermInformation.getDocumentationsDate())+". "+ermInformation.getTransferAmount2());
			    para2.add(text7); 
			    }else if(ermInformation.getTimeType().equalsIgnoreCase("Complete")) {
			    	Text text7 =new Text ("\n d) The subscriber has provided "+ermInformation.getTimeType()+" set of documents on " +NpstCommonUtil.getDateString(ermInformation.getDocumentationsDate()) +". "+ermInformation.getTransferAmount2());
				para2.add(text7);	
			    }else {
			    ...
			    String[][] cellContent1 =	{
			    		...
			    		{"12","Rectification Request Mode (CGMS/Direct Mail-Letter from sub/Req from NPST-PFRDA)",rectificReqMode},
