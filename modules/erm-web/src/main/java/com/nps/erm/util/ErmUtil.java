...
import com.nps.erm.service.ErmBatchInformationLocalService;
...
@Component(immediate = true, service = ErmUtil.class)
public class ErmUtil {
	...
	private  String getExcelFormattedValue(String value) {
	...
	public  List<DLFileEntry> getFileEntries() {
			...
			dlFileEntries=dlFileEntries.stream().sorted(Comparator.comparing(DLFileEntry::getCreateDate).reversed()).collect(Collectors.toList());
return dlFileEntries;
	}
	
	public  File	exportExcel(List<ErmModal> ermModals){
		File file = null;
		try {
			 file=FileUtil.createTempFile("Export.csv");
			 Set<String> duplicatePran=getDuplicatePran();
	...
	public  File exportToExcel2(ErmBatchInformation batchInformation) {
		File file = null;
		try {
			...
			for(String ermId:batchInformation.getErmInformationIds().split(StringPool.COMMA)) {
				  try {
					  sb.setIndex(sb.index() - 1);
						...
						sb.append(getExcelFormattedValue(String.valueOf(++index)));
						sb.append(COMMA);
					  ErmInformation ermInformation= ermInformationLocalService.getErmInformation(Long.valueOf(ermId));
						try {
	...
	public  File exportToExcel1(ErmBatchInformation batchInformation) {
		File file = null;
		try {
...
//				sb.append(CharPool.NEW_LINE);
				...
				sb.append(COMMA);
				sb.append(getExcelFormattedValue(String.valueOf(getReceiptDates(batchInformation.getErmBatchInformationId())).replace(COMMA, "")));
				sb.append(COMMA);
				sb.append(CharPool.NEW_LINE);
	...
	public  long getDaysBetween(Date from,Date to){
		...
		LocalDate end = to.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
	        ...
	        logger.debug("Date difference is :"+totalDays);
	        return totalDays;
	}
	 ...
	 * @return
	 */
	public  long getBusinessDaysBetween(Date from,Date to){
	...
	public  long getMaxBatchValue() {
		DynamicQuery dynamicQuery =ermBatchInformationLocalService.dynamicQuery() .setProjection(ProjectionFactoryUtil.max("batchNo"));
     //   DynamicQuery dynamicQuery = DynamicQueryFactoryUtil.forClass(ErmBatchInformation.class,ClassLoader.getSystemClassLoader()).setProjection(ProjectionFactoryUtil.max("batchNo"));
        ...
        List<Long> result = ermInformationLocalService.dynamicQuery(dynamicQuery);
	 ...
	 * @return
	 */
	public  List<WorkflowTask> getCRAPendingBatchWorkflowTasks(long companyId,boolean isCompleted){
	 ...
	 * @return
	 */
	public  List<WorkflowTask> getMyAndMyRoleWorkflowTasks(long companyId,long userId,boolean isCompleted){
		List<WorkflowTask> list=null;
		try {
		 list=new ArrayList<WorkflowTask>();
 ...
 * @return
 */
public  List<ErmBatchModal> getMyAndMyRolesBatch(long companyId,long userId){
 ...
 * @return
 */
public  List<ErmBatchModal> getCRAPendinBatch(long companyId,long userId){
 ...
 * @return
 */
public  List<ErmBatchModal>  getErmBatch(long companyId,long userId) {
 ...
 */
public  void performWorkflowTasks(String[] ermIds, String action, String remarks,String process,String recommendation, long userId,long companyId) throws PortalException {
	...
	for (String ermId : ermIds) {
		...
		ermInformation=ermInformationLocalService.updateErmInformation(ermInformation);
		logger.info("#####################################");
 ...
 * @throws PortalException
 */
public  void resubmitForm(long ermInformationId, String action, String remarks, long userId) throws PortalException {
	ErmInformation ermInformation=ermInformationLocalService.getErmInformation(ermInformationId);	
	performWorkFlowACtion(ermInformation, action, remarks, userId);
}
 ...
 * @throws PortalException
 */
private   void performWorkFlowACtion(ErmInformation ermInformation, String action, String remarks, long userId)throws PortalException {
	...
	if (workflowLogs_assign.size() > 0) {
		...
		for(String trName:nextTransitionNames) {
			...
			if (task.getAssigneeUserId() == userId) {
				logger.info("task self assignee user id:  "+task.getAssigneeUserId()+"  performer userid: "+userId);
...
public  List<ErmModal> getErmModalsByCRA(long companyId,long userId ,String craName) throws PortalException{
	...
	if(craName.equalsIgnoreCase(NpstErmConstant.ALL_CRA) || craName==NpstErmConstant.ALL_CRA) {
		ermInformations=ermInformationLocalService.getErmInformations(-1, -1);
	}else {
		ermInformations=ermInformationLocalService.findByPfmName(craName);
	}
 ...
 * @return
 */
public  Set<String> getDuplicatePran() {
	DynamicQuery dynamicQuery= ermInformationLocalService.dynamicQuery().setProjection(ProjectionFactoryUtil.property("pran"));
	List list=ermInformationLocalService.dynamicQuery(dynamicQuery);
...
//	List<ErmInformation> ermInformations=ermInformationLocalService.getErmInformations(QueryUtil.ALL_POS, QueryUtil.ALL_POS);
 ...
 * @throws PortalException
 */
	public   List<ErmModal> getErmModalBYBatchNo(String batchNo,long companyId, long userId)throws PortalException {
		logger.info("batchNo :-  "+batchNo);
		List<ErmModal> ermModals=new ArrayList<ErmModal>();
		ErmBatchInformation ermBatchInformation=ermBatchInformationLocalService.getErmBatchInformation(Long.valueOf(batchNo));
		//ErmBatchInformation ermBatchInformation=ermBatchInformationLocalService.findByBatchNo(Long.valueOf(batchNo));
		//List<ErmInformation> ermInformations=ermInformationLocalService.findByBatch(batchNo);
		 ...
		 * @throws PortalException
		 */
		public   List<ErmModal> getErmModalByPran(long companyId,long groupId,long userId,String pran)throws PortalException {
			List<ErmModal> ermModals=new ArrayList<ErmModal>();
			List<ErmInformation> ermInformations=ermInformationLocalService.findByPran(groupId, pran);
			for(ErmInformation ermInformation:ermInformations) {
			try {
		...
		public   List<ErmModal> getErmByUserId(long companyId,long userId)throws PortalException {
			List<ErmModal> ermModals=new ArrayList<ErmModal>();
			List<ErmInformation> ermInformations=ermInformationLocalService.findByUserId(userId);
			for(ErmInformation ermInformation:ermInformations) {
			try {
			ermModals.add(getErmModal(ermInformation, companyId, userId));
			}catch (Exception e) {
	 ...
	 * @return
	 */
	public  List<ErmModal> getBatchCreationRecords(long companyId,long userId){
	...
	public  List<ErmModal> getErmStatusModal(long companyId,long userId){
		boolean isNpstUser=NpstCommonUtil.isNpstUser(userId, companyId);
		List<ErmModal> ermModals=getErms(companyId, userId);
...
//			ermModals=ermModals.stream().map(erm->erm).collect(Collectors.toSet()).stream().map(erm->erm).collect(Collectors.toList());
	...
	public  List<ErmModal> getApprovedErm(long companyId,long userId) {
		List<ErmModal> ermModals=getErms(companyId, userId);
	 ...
	 * @param userId
	 */
	public  List<ErmModal> getAssignableErm(long companyId,long userId) {
		List<ErmModal> ermModals=getErms(companyId, userId);
	 ...
	 */
	public   ErmModal getErmModal(ErmInformation ermInformation,long companyId,long userId) throws WorkflowException {
...
//		}
		...
		logger.debug("workflow task id:  "+workflowTask);
		...
		logger.info("currentlyAssignedUserName :"+currentlyAssignedUserName+"  isCompleted: "+isCompleted +"  pran: "+ermInformation.getPran()+"  craname: "+ermInformation.getPfmName());
 ...
 * @return
 */
private  List<ErmBatchModal> getErmBatchModals(List<WorkflowTask> workflowTasks,long companyId,long userId ){
	List<ErmBatchModal>ermBatchModals=new ArrayList<ErmBatchModal>();
	List<Long> ermIds=new ArrayList<Long>();
	workflowTasks=workflowTasks.stream().sorted(Comparator.comparing(WorkflowTask::getCreateDate).reversed()).collect(Collectors.toList());
	...
	for (WorkflowTask itr : workflowTasks) {
		...
		ermInformation = ermInformationLocalService.getErmInformation(applicationId);
		//ermBatchInformation=ermBatchInformationLocalService.getErmBatchInformation(Long.valueOf(ermInformation.getBatchNo()));
		 ermBatchInformation=ermBatchInformationLocalService.findByBatchNo(Long.valueOf(ermInformation.getBatchNo()));
		 String batchStatus="Confirmation received from CRA";
		 for(String ermId:ermBatchInformation.getErmInformationIds().split(StringPool.COMMA)) {
			try {
			ErmInformation erm=	ermInformationLocalService.getErmInformation(Long.valueOf(ermId));
	 ...
	 * @throws PortalException
	 */
		public   List<ErmBatchModal> getAllErmBatchModal(long companyId, long userId)throws PortalException {
			List<ErmBatchModal> ermBatchModals=new ArrayList<ErmBatchModal>();
			
			for(ErmBatchInformation ermBatchInformation:ermBatchInformationLocalService.getErmBatchInformations(QueryUtil.ALL_POS,QueryUtil.ALL_POS) ) {
				logger.info("ermBatchInformation.getErmBatchInformationId()   :  "+ermBatchInformation.getErmBatchInformationId());
				//List<ErmInformation> ermInformations=ermInformationLocalService.findByBatch(ermBatchInformation.getErmBatchInformationId()+"");
				try {
					for(String ermId:ermBatchInformation.getErmInformationIds().split(StringPool.COMMA)) {
						try {
						ErmInformation ermInformation=ermInformationLocalService.getErmInformation(Long.valueOf(ermId));
 ...
 * @throws WorkflowException 
 */
private  ErmModal createErmModal(long companyId,long userId,ErmInformation ermInformation,long workflowTaskId,String comment,String intermediaryName,String currentlyAssignedUserName,String dueDate,boolean isSelfAsignee,boolean isAssignable,boolean isCompleted) throws WorkflowException{
	...
	try {
		...
		long subcriberCheck=	getDaysBetween(ermInformation.getTransactionDate(), ermInformation.getDocumentationsDate());
		long craCheck=	getBusinessDaysBetween(ermInformation.getDocumentationsDate(), ermInformation.getFwdnpstDate());
		if(subcriberCheck>7) {
 ...
 * @throws WorkflowException 
 */
private   ErmBatchModal createErmBatchModal(long companyId,long userId,ErmBatchInformation ermBatchInformation,long workflowTaskId,String currentlyAssignedUserName,String dueDate, boolean isSelfAsignee,String batchStatus) throws WorkflowException{
 ...
 * @return
 */
private   List<ErmModal> getErms(long companyId,long userId) {
	List<ErmModal> ermModals=new ArrayList<ErmModal>();
	List<ErmInformation> ermInformations=null;
	boolean isNpstUser=NpstCommonUtil.isNpstUser(userId, companyId);
	/////////////////////////////////
	if(isNpstUser) {
		 ermInformations=ermInformationLocalService.getErmInformations(QueryUtil.ALL_POS, QueryUtil.ALL_POS);
	}else {
		String craName=NpstCommonUtil.getExpandoValue(companyId, userId, User.class.getName(), NpstCommonConstant.GRIVANCES_TYPE);
		 ermInformations=ermInformationLocalService.findByPfmName(craName);
	}
	...
	for(ErmInformation ermInformation: ermInformations) {
		try {
			logger.debug("pran no.  :-"+ermInformation.getPran()+"   pfm:  "+ermInformation.getPfmName());
			ermModals.add(getErmModal(ermInformation,companyId,userId));
		}catch (Exception e) {
 ...
 * @return
 */
public  String getReceiptDates(long batchId) {
	Set<String> set=new HashSet<String>();
	String receiptDate="";
	try {
	ErmBatchInformation batchInformation=ermBatchInformationLocalService.getErmBatchInformation(batchId);
 ...
 * @return
 */
public  boolean isAssignable(long companyId,long userId, long taskId) {
	try {
 ...
 * @return
 */
public  String getAssigneeName(long classPK, String className,long companyId,String craName) {
	long userId = 0;
	String assigneeName="";
	 ...
	 * @return
	 */
	private  List<KaleoLog> getKaleoLogs(long companyId, long workflowInstanceId,String logType) {
	 ...
	 * @return
	 */
	private  List<WorkflowLog> getWorkflowLogs(long companyId,long groupId,String className,long pk,List<Integer> logTypes){
	 ...
	 * @return
	 */
	private  String getComment(List<KaleoLog> kaleoLogs) {
	 ...
	 * @return
	 */
	private  boolean isSelfAsignee(List<KaleoLog> kaleoLogs,long userId) {
		boolean isSelfAsignee=false;
 ...
 * @throws PortalException
 */
private  Map<String, Serializable> getWorkflowContext(long companyId,long groupId,String className,long pk) throws PortalException{
	WorkflowInstanceLink wil = WorkflowInstanceLinkLocalServiceUtil.getWorkflowInstanceLink(
			companyId, groupId, className,pk);
...
public static void sendEmailToERMUsers(String status,long id,String roleName,long companyId) {
	...
	try {
	ErmInformation ermInformation= ErmInformationLocalServiceUtil.fetchErmInformation(id);
	String body ="";
	String subject = "ERM request of "+ermInformation.getSubscriberName()+" bearing PRAN "+ermInformation.getPran();
	
	//ServiceContext serviceContext=ServiceContextThreadLocal.getServiceContext();
	//logger.info("serviceContext : "+serviceContext);
	//long userid=serviceContext.getUserId();
	long currentUserId=PrincipalThreadLocal.getUserId();
	boolean isNpstUser=NpstCommonUtil.isNpstUser(currentUserId, companyId);
	//String userName=UserLocalServiceUtil.fetchUser(userid).getFullName();
	logger.info("isNpstUser: "+isNpstUser+"  currentUserId : "+currentUserId);
	if((ermInformation.getIsForwardToNpst()==1) && (roleName.equalsIgnoreCase("NPSTAM-MGR-Grievances") || roleName.equalsIgnoreCase("NPSTDGM-Grievances"))) {
		if(ermInformation.getErmStatus().equalsIgnoreCase("Confirmed by CRA")) {
			
			body = "<p>Dear Sir/Madam,</p>" 
					+ "<p>ERM request of "+ermInformation.getSubscriberName()+" bearing PRAN "+ermInformation.getPran()+" has been processed in the CRA portal.</p>" 
					+ "<p>This email is system generated. Hence, no reply is required.</p>"
					+"<p>Regards,</p>"
					+"<p>NPS Trust.</p>";
		}else {
			String userName=UserLocalServiceUtil.fetchUser(currentUserId).getFullName();
			body = "<p>Dear Sir/Madam,</p>" 
					+ "<p>ERM request of "+ermInformation.getSubscriberName()+" bearing PRAN "+ermInformation.getPran()+" has been submitted by "+userName +" from "+ermInformation.getPfmName().replace(NameMappingConstants.NCRA_OLD, NameMappingConstants.NCRA_NEW).replace(NameMappingConstants.CAMS_OLD, NameMappingConstants.CAMS_NEW)+" for your review.</p>" 
					+ "<p>This email is system generated. Hence, no reply is required.</p>"
					+"<p>Regards,</p>"
					+"<p>NPS Trust.</p>";
		}
	}if(roleName.equalsIgnoreCase(ermInformation.getPfmName()+"-Maker") || roleName.equalsIgnoreCase(ermInformation.getPfmName()+"-Checker")) {
		
		if(isNpstUser) {
			if(ermInformation.getBankName()==""||ermInformation.getBankName()==null || ermInformation.getBankName().equalsIgnoreCase("")) {
				String userName=UserLocalServiceUtil.fetchUser(ermInformation.getUserId()).getFullName();
				body = "<p>Dear Sir/Madam,</p>" 
						+ "<p>ERM request of "+ermInformation.getSubscriberName()+" bearing PRAN "+ermInformation.getPran()+" has been reverted by NPS Trust for required modification. Please ensure that the necessary changes are made in this workflow itself by the  "+userName +".</p>" 
						+ "<p>This email is system generated. Hence, no reply is required.</p>"
						+"<p>Regards,</p>"
						+"<p>NPS Trust.</p>";	
			}else {
				body = "<p>Dear Sir/Madam,</p>" 
						+ "<p>ERM request of "+ermInformation.getSubscriberName()+" bearing PRAN "+ermInformation.getPran()+" has been "+(status.equalsIgnoreCase("Review")?"approved":"rejected")+" by NPS Trust For details, please check in your DCMS login.</p>" 
						+ "<p>This email is system generated. Hence, no reply is required.</p>"
						+"<p>Regards,</p>"
						+"<p>NPS Trust.</p>";	
			}
		}else if(roleName.equalsIgnoreCase(ermInformation.getPfmName()+"-Checker")) {
			String makerName=UserLocalServiceUtil.fetchUser(ermInformation.getUserId()).getFullName();
			body = "<p>Dear Sir/Madam,</p>" 
					+ "<p>ERM request of "+ermInformation.getSubscriberName()+" bearing PRAN "+ermInformation.getPran()+" has been submitted by "+makerName+" for your review and further submission to NPS Trust.</p>" 
					+ "<p>This email is system generated. Hence, no reply is required.</p>"
					+"<p>Regards,</p>"
					+"<p>NPS Trust.</p>";	
		}
	...
	try {
		...
		for(User lr_user : list_of_users) {
			...
			sendEmail("noreply@npstrust.net", lr_user.getEmailAddress().trim(), subject, body);
			}
			//emailMap.put(lr_user.getEmailAddress().trim(),lr_user.getFullName());
		}
	} catch (PortalException e) {
		logger.error(e);
	}
...
//public void sendErmEmail(long companyId, String roleName,String pfmName, String userName, String entityName, String action) {
//
...
//private Map<String,String> getEmail(long companyId, String role_name) {
//	Map<String,String> emailMap = new HashedMap<>();
//	try {
...
public static void sendEmail(String from, String to, String subject, String body) {
	try {
		...
		try {
			...
			logger.info("Mail sent to user at ::=> " + to +"    body:  "+body);
			//_log.info("Mail sent to bcc at ::=> noreply@npstrust.net");

		} catch (AddressException e1) {
			logger.error("Mail sent to 'add' 'lp' User failed ::", e1);
		}
	} catch (Exception e) {
		logger.error("Email notification could not be sent", e);
	}

}
{
	objCount=objCount+1;
	logger.info("object created "+objCount);
	ermInformationLocalService=	ErmInformationLocalServiceUtil.getService();
	ermBatchInformationLocalService=ErmBatchInformationLocalServiceUtil.getService();
}
...
@Reference
private ErmBatchInformationLocalService ermBatchInformationLocalService;
