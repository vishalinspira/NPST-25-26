					...
					<tr>
					 <portlet:actionURL name="updateBatch" var="updateBatchURL" >
						...
						<td class="tbdata"><%if(ermModal.isAssignable() && isCancelable && ermInformation.getBatchStatus()!=NpstErmConstant.NPST_FIRST_LEVEL_APPROVED){%>
							<input class="comment" id="comment<%=ermInformation.getErmInformationId() %>" name="comment<%=ermInformation.getErmInformationId() %>" value="<%=HtmlUtil.escape(ermInformation.getNpstRemark()) %>" label="" type="text"></input>
							<%}else{ %>
							<%=HtmlUtil.escape(ermInformation.getNpstRemark()) %>
							<%} %>
							</td>
