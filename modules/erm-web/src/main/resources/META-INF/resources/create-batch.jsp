...
<table id="createBatchTable" class="dataTable-table table table-bordered table-responsive">
		...
		<tbody>
					...
					<%-- 	<td><%=count++ %></td> --%>
						...
						<td class="tbdata">
						<a style="color: #6b6c7e;" target="_blank" href="<%= ermModal.isAssignable()?editErmURL:'#'%>"><%=HtmlUtil.escape(ermInformation.getSubscriberName()) %></a>						 
					...
					<%-- 	<input class="subcriberSector" placeholder="This is required." name="<%=ErmFieldName.SUBSCRIBER_SECTOR %><%=ermInformation.getErmInformationId() %>" id="<%=ErmFieldName.SUBSCRIBER_SECTOR %><%=ermInformation.getErmInformationId() %>" label="" type="text" value="<%=ermInformation.getSubscriberSector() %>"></input> --%>
						...
						<td class="tbdata">
						<input class="remark" id="remark<%=ermInformation.getErmInformationId() %>" name="remark<%=ermInformation.getErmInformationId() %>" label="" type="text" value="<%=HtmlUtil.escape(ermInformation.getNpstRemark()) %>"></input>
						</td>
						<td class="tbdata">
