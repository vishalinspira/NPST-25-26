<%@page import="com.nps.erm.service.ErmInformationLocalService"%>
...
<%@page import="com.liferay.portal.kernel.portlet.LiferayWindowState"%>
 ...
 <%
 ErmInformationLocalService ermInformationLocalService=(ErmInformationLocalService)request.getAttribute("ermInformationLocalService");
 ...
 if(ermInfoId<=0){
	 ermInformation=ermInformationLocalService.createErmInformation(0);
 }else{
	 ermInformation=ermInformationLocalService.getErmInformation(ermInfoId);
 }
...
<aui:input readonly="true" label="New" name="<%=ErmFieldName.BATCH_TYPE %>" type="radio" value="<%=ErmConstantValues.NEW_BATCH_TYPE %>" checked="<%=ermInformation.getBatchType().equals(ErmConstantValues.NEW_BATCH_TYPE)?true:true %>" />
 ...
 a)	The Subscriber made a contribution of Rs.  <div style=" width: 12rem; display: inline-block;">
  <aui:input readonly="true" type="text" name="<%=ErmFieldName.TRANSACTED_AMOUNT %>"  id="<%=ErmFieldName.TRANSACTED_AMOUNT %>" label="" value="<%=ermInformation.getTransactedAmount() %>"> 
   ...
   <aui:input type="text" readonly="true" name="<%=ErmFieldName.PRAN %>" id="<%=ErmFieldName.PRAN %>" label="" value="<%= HtmlUtil.escape(ermInformation.getPran())%>" >
  </aui:input>
  ...
  <aui:input type="text" readonly="true"  label="" name="<%=ErmFieldName.TRANSACTION_MODE %>" value="<%= HtmlUtil.escape(ermInformation.getTransactionMode())%>" ></aui:input>
...
<aui:input type="text" readonly="true"  label="" name="<%=ErmFieldName.RECTIFICATION_REQUEST_MODE %>" value="<%= HtmlUtil.escape(ermInformation.getRectificationRequestMode())%>" ></aui:input>
   ...
   dated <div style=" width: 12rem; display: inline-block;"><aui:input type="date" readonly="true" name="<%=ErmFieldName.RECTIFICATION_DATE %>" id="<%=ErmFieldName.RECTIFICATION_DATE %>" label=""  value="<%=ermInformation.getRectificationDate() != null ? simpleDateFormat.format(ermInformation.getRectificationDate()) : null%>">
    ...
    on <div style=" width: 12rem; display: inline-block;"><aui:input type="date" readonly="true" name="<%=ErmFieldName.REMITTED_DATE %>" id="<%=ErmFieldName.REMITTED_DATE %>" label="" value="<%=ermInformation.getRemittedDate() != null ? simpleDateFormat.format(ermInformation.getRemittedDate()) : null%>">
      ...
      in Tier I <div style=" width: 12rem; display: inline-block;">
       <aui:input type="text" name="<%=ErmFieldName.TIER_TYPE %>" readonly="true" id="<%=ErmFieldName.TIER_TYPE %>"  label="" value="<%=HtmlUtil.escape(ermInformation.getTierType()) %>"></aui:input>

</div>
   Hence subscriber has requested to
   <div style=" width: 12rem; display: inline-block;">
   <aui:input type="text" readonly="true"  label="" name="<%=ErmFieldName.TRANSACTION_TYPE %>" value="<%= HtmlUtil.escape(ermInformation.getTransactionType())%>" ></aui:input>
</div>
 the amount of    Rs. 
 <div style=" width: 8rem; display: inline-block;"> 
 ...
 <div style=" width: 8rem; display: inline-block;"> 
  <aui:input type="text" name="<%=ErmFieldName.TRANSFER_AMOUNT1 %>" readonly="true" id="<%=ErmFieldName.TRANSFER_AMOUNT1 %>"  label="" value="<%=HtmlUtil.escape(ermInformation.getTransferAmount1()) %>"> 
 </aui:input> 
</div>

<div>
d)	The subscriber has provided  <div style=" width: 12rem; display: inline-block;">
<aui:input type="text" name="<%=ErmFieldName.TIME_TYPE %>" readonly="true" id="<%=ErmFieldName.TIME_TYPE %>" label="" value="<%=ermInformation.getTimeType()%>" > </aui:input>

</div> set of documents on
<%if(ermInformation.getTimeType().equalsIgnoreCase("Initial")){ %>
 <div id="timeTypeDIV" style=" width: 25rem; display: inline-block;">
<div style=" width: 8rem; display: inline-block;">
<aui:input type="date" name="<%=ErmFieldName.TIME_TYPE_DATE %>" readonly="true" id="<%=ErmFieldName.TIME_TYPE_DATE %>" label="" value="<%=ermInformation.getTimeTypeDate() != null ? simpleDateFormat.format(ermInformation.getTimeTypeDate()) : null%>" >
</aui:input>
</div>
<div style=" width: 16rem; display: inline-block;">
and complete set of documents on
</div>
</div>
<%} %>

 <div style=" width: 8rem; display: inline-block;"><aui:input type="date" readonly="true" name="<%=ErmFieldName.DOCUMENTATIONS_DATE %>" id="<%=ErmFieldName.DOCUMENTATIONS_DATE %>" label="" value="<%=ermInformation.getDocumentationsDate() != null ? simpleDateFormat.format(ermInformation.getDocumentationsDate()) : null%>" >
</aui:input>
 </div>.

<div style=" width: 15rem; display: inline-block;"> 
  <aui:input type="text" name="<%=ErmFieldName.TRANSFER_AMOUNT2 %>" readonly="true" id="<%=ErmFieldName.TRANSFER_AMOUNT2 %>" placeholder="Please enter comments, if any" label="" value="<%=HtmlUtil.escape(ermInformation.getTransferAmount2()) %>"> 
 </aui:input> </div>
</div>
<div>
...
d)	The subscriber has provided <div style=" width: 8rem; display: inline-block;"> 
  <aui:input type="text" name="<%=ErmFieldName.TRANSFER_AMOUNT2 %>" readonly="true" id="<%=ErmFieldName.TRANSFER_AMOUNT2 %>"  label="" value="<%=HtmlUtil.escape(ermInformation.getTransferAmount2()) %>"> 
...
<h1>Section B: Transaction Details</h1>
 ...
 <table id="transactionDetailsTable"
	    ...
	    <tr>
			 ...
			 <td class="tbdata">
			 <aui:input type="text" readonly="true" name="<%=ErmFieldName.CASE_NO %>" id="<%=ErmFieldName.CASE_NO %>" label="" value="<%=HtmlUtil.escape(ermInformation.getCaseNo()) %>"></aui:input>
		...
		<tr>
			 ...
			 <td  class="tbdata"><%=ErmFieldLabel.TABLE_VIEW_PRAN %> </td>
			  <td class="tbdata">
             <aui:input type="text" name="<%=ErmFieldName.AUTO_PRAN %>" id="<%=ErmFieldName.AUTO_PRAN %>" readonly="true" label="" value="<%=HtmlUtil.escape(ermInformation.getPran()) %>"></aui:input>
			 </td>
		</tr>		
			<tr>
			 <td>3</td>
			 ...
			 <td class="tbdata">
             <aui:input type="text" readonly="true" name="<%=ErmFieldName.SUBSCRIBER_NAME %>" label="" value="<%=HtmlUtil.escape(ermInformation.getSubscriberName()) %>"></aui:input>
			...
			<tr>
			 ...
			 <td  class="tbdata"><%=ErmFieldLabel.TABLE_VIEW_TRANSACTION_MODE %> </td>
			  <td class="tbdata">
             <aui:input type="text" name="<%=ErmFieldName.AUTO_TRANSACTION_MODE %>" id="<%=ErmFieldName.AUTO_TRANSACTION_MODE %>" value="<%=HtmlUtil.escape(ermInformation.getTransactionMode()) %>" label="" readonly="true"></aui:input>
		...
		<tr>
			 ...
			 <td  class="tbdata"><%=ErmFieldLabel.TABLE_VIEW_ENPS_ORDER_ID %></td>
			  <td class="tbdata">
			  <aui:input type="text" name="<%=ErmFieldName.ENPS_ORDER_ID %>" readonly="true" label="" value="<%=HtmlUtil.escape(ermInformation.getEnpsOrderId()) %>">
			</aui:input>
		...
		<tr>
			...
			<tr>
             ...
             <td class="tbdata">
             <aui:input type="text" readonly="true"  label="" name="<%=ErmFieldName.RECTIFICATION_TYPE %>" value="<%= HtmlUtil.escape(ermInformation.getRectificationType())%>" ></aui:input>
			 </td>
		</tr>
		<tr>
			 <td>11</td>
		...
		<tr>
             ...
             <td class="tbdata">
            <aui:input type="text" name="<%=ErmFieldName.AUTO_RECTIFICATION_REQUEST_MODE %>" readonly="true" id="<%=ErmFieldName.AUTO_RECTIFICATION_REQUEST_MODE %>" label=""  value="<%=HtmlUtil.escape(ermInformation.getRectificationRequestMode()) %>"></aui:input>
...
b)	Grievance Text raised by Subscriber :<div style=" width: 60rem; display: inline-block;"><aui:input type="textarea" readonly="true" name="<%=ErmFieldName.GRIEVANCE_TEXT %>"  label="" value="<%=HtmlUtil.escape(ermInformation.getGrievanceText()) %>">
</aui:input></div>
