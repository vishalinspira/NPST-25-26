...
List<ErmBatchModal> ermBatchModals= ermUtil.getCRAPendinBatch(user.getCompanyId(), user.getUserId());
...
<portlet:renderURL var="backURL1" windowState="<%=LiferayWindowState.NORMAL.toString()%>">
...
<a href="<%=backURL1%>" class="btn btn-primary" style="border-radius: 50px;"><i class="fas fa-arrow-left text-white pr-3"></i>Back</a>
</div>
...
<table id="pendingBatchTable" class="dataTable-table table table-bordered table-responsive" style="width:100%">
		...
		<tbody>
					...
					ErmInformation ermInformation=ermInformationLocalService.getErmInformation(Long.parseLong(batchInformation.getErmInformationIds().split(StringPool.COMMA)[0]));
						%>
