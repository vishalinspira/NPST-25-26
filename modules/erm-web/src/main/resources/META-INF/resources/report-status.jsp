...
<%
ErmUtil ermUtil=(ErmUtil)request.getAttribute("ermUtil"); 
...
List<ErmModal> ermModals= ermUtil.getErmStatusModal(user.getCompanyId(), user.getUserId());
//List<ErmInformation> ermModals= ermInformationLocalService.getErmInformations(-1, -1);
...
<div style="width: 10%; float: right; margin-top: -2rem;">
...
<portlet:resourceURL var="exportURL" id="/erm/excel/export">
						</portlet:resourceURL>

						<a class="btn btn-primary" style="border-radius: 50px;" href="<%=exportURL%>">Export</a>
</div>
<%} %>
...
<table id="reportStatusTable" class="dataTable-table table table-bordered table-responsive">
		<thead>
			<tr class="border">
				<th>Sr.No</th>
		...
		<tbody>
					...
					<tr>
        				...
        				<td><%=count++ %></td> 
						<td class="tbdata"><%=HtmlUtil.escape(ermModal.getIntermediaryName()) %></td>
						<%-- <td class="tbdata"><%=ermInformation.getPran() %></td> --%>
						<td class="tbdata"><a target="_blank" href="<%=viewErmURL%>"><%=ermInformation.getPran() %></a></td>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getSubscriberName()) %></td>
						...
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getRectificationType()) %></td>
						...
						<%if(isNpstUser){ %>
							...
							<%} %>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getNoNpstRemark()) %></td>
						<%if(isNpstUser){ %>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getBankName()) %></td>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getBankStatmentName()) %></td>
						...
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getSubscriberSector()) %></td>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getSubmissionStipulatedTime()) %></td>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getStipulated()) %></td>
						
						<%} %>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getNpstRemark()) %></td>
						...
						<%} %>
						<td class="tbdata"><%=HtmlUtil.escape(ermInformation.getRecommendation()) %></td>
						<td><%=ermModal.getAssignedTo() %></td>
						<%if(isNpstUser){ %>
