...
<%@page import="com.liferay.portal.kernel.portlet.LiferayWindowState"%>
 ...
 <portlet:actionURL name="saveErmInformation" var="saveErmURL" />
  ...
  <aui:form action="<%=saveErmURL %>" name="ermForm" id="ermForm" method="POST" >
...
<aui:input label="New" name="<%=ErmFieldName.BATCH_TYPE %>" type="radio" value="<%=ErmConstantValues.NEW_BATCH_TYPE %>" checked="<%=ermInformation.getBatchType().equals(ErmConstantValues.NEW_BATCH_TYPE)?true:true %>" />
 ...
 a)	The Subscriber made a contribution of Rs.  <div style=" width: 12rem; display: inline-block;">
  ...
  <aui:validator name="digits"/> 
   ...
   <aui:input type="text" name="<%=ErmFieldName.PRAN %>" id="<%=ErmFieldName.PRAN %>" label="" value="<%= HtmlUtil.escape(ermInformation.getPran())%>" >
 <aui:validator name="digits"/>
  <aui:validator name="required" />
...
<aui:validator  name="custom"  errorMessage="<%=NpstErmConstant.DATE_ERROR_MESSAGE %>" >
 ...
 <aui:validator name="digits"/>
	...
	return dateValidate(val);
      ...
      in Tier I <div style=" width: 12rem; display: inline-block;">
       <aui:input type="text" name="<%=ErmFieldName.TIER_TYPE %>" placeholder="Enter Text" id="<%=ErmFieldName.TIER_TYPE %>"  label="" value="<%=HtmlUtil.escape(ermInformation.getTierType()) %>">
       <aui:validator name="required" />
       </aui:input>
 ...
 <div style=" width: 8rem; display: inline-block;"> 
  <aui:input type="text" name="<%=ErmFieldName.TRANSFER_AMOUNT1 %>" id="<%=ErmFieldName.TRANSFER_AMOUNT1 %>" placeholder="Enter text" label="" value="<%=HtmlUtil.escape(ermInformation.getTransferAmount1()) %>"> 
 </aui:input> 
</div>
...
d)	The subscriber has provided  <div style=" width: 12rem; display: inline-block;">
      <aui:select name="<%=ErmFieldName.TIME_TYPE %>" label="" required="true" id="<%=ErmFieldName.TIME_TYPE %>" value="<%=ermInformation.getTimeType() %>">
  <aui:option value="">Select</aui:option>
 <%for(Entry<String,String> entry:ErmConstantValues.TIME_TYPE_MAP.entrySet()){ 
	 						boolean isSelected = false;
							if ((ermInformation.getTimeType() == entry.getKey())|| (ermInformation.getTimeType().equals(entry.getKey()))){
							isSelected = true;}%>
                        <aui:option value="<%=entry.getKey() %>" selected="<%=isSelected %>"><%=entry.getValue() %></aui:option>
                        <%} %>
</aui:select>
</div> set of documents on <div id="timeTypeDIV" style=" width: 25rem; display: none;">
<div style=" width: 8rem; display: inline-block;">
<aui:input type="date" name="<%=ErmFieldName.TIME_TYPE_DATE %>" id="<%=ErmFieldName.TIME_TYPE_DATE %>" label="" value="<%=ermInformation.getTimeTypeDate() != null ? simpleDateFormat.format(ermInformation.getTimeTypeDate()) : null%>" >
 <aui:validator name="daterequired" errorMessage="This field is required." >
 function (val, fieldNode, ruleValue) {
		    var timetype=$("#<portlet:namespace /><%=ErmFieldName.TIME_TYPE%>").val();
	if(timetype=='Complete'){
	return true;
	}else{
	return false;
	}
  ...
  <aui:validator  name="custom"  errorMessage="cannot be before transaction date." >
	...
	var date2 = new Date(transaction_date).getTime();
	console.log("good");
	if (date2 <= date1 || timetype=='Complete') {
	result = true;
	}
	return result;
...
and complete set of documents on
 ...
 <div style=" width: 8rem; display: inline-block;"><aui:input type="date" name="<%=ErmFieldName.DOCUMENTATIONS_DATE %>" id="<%=ErmFieldName.DOCUMENTATIONS_DATE %>" label="" value="<%=ermInformation.getDocumentationsDate() != null ? simpleDateFormat.format(ermInformation.getDocumentationsDate()) : null%>" >
 <aui:validator name="required" />
  <aui:validator  name="custom"  errorMessage="cannot be before transaction date." >
	function (val, fieldNode, ruleValue) {
		    var transaction_date=$("#<portlet:namespace /><%=ErmFieldName.TRANSACTION_DATE%>").val();
...
<div style=" width: 15rem; display: inline-block;"> 
  <aui:input type="text" name="<%=ErmFieldName.TRANSFER_AMOUNT2 %>" id="<%=ErmFieldName.TRANSFER_AMOUNT2 %>" placeholder="Please enter comments, if any" label="" value="<%=HtmlUtil.escape(ermInformation.getTransferAmount2()) %>"> 
 </aui:input> </div>
</div>
...
<h1>Section B: Transaction Details</h1>
 ...
 <table id="transactionDetailsTable"
	    ...
	    <tr>
			 ...
			 <td class="tbdata">
			 <aui:input type="text" name="<%=ErmFieldName.CASE_NO %>" id="<%=ErmFieldName.CASE_NO %>" label="" value="<%=HtmlUtil.escape(ermInformation.getCaseNo()) %>">
			  <aui:validator name="required" />
		...
		<tr>
			...
			<tr>
			 ...
			 <td class="tbdata">
             <aui:input type="text" name="<%=ErmFieldName.SUBSCRIBER_NAME %>" label="" value="<%=HtmlUtil.escape(ermInformation.getSubscriberName()) %>">
              <aui:validator name="required" />
			</aui:input>
		...
		<tr>
			 ...
			 <td  class="tbdata"><%=ErmFieldLabel.TABLE_VIEW_ENPS_ORDER_ID %></td>
			  <td class="tbdata">
			  <aui:input type="text" name="<%=ErmFieldName.ENPS_ORDER_ID %>" label="" value="<%=HtmlUtil.escape(ermInformation.getEnpsOrderId()) %>">
			   <aui:validator name="required" />
			 <aui:validator name="alphanum"/>
			</aui:input>
	...
	return result;
		...
		<tr>
             ...
             <td class="tbdata">
            <aui:input type="text" name="<%=ErmFieldName.AUTO_RECTIFICATION_REQUEST_MODE %>" id="<%=ErmFieldName.AUTO_RECTIFICATION_REQUEST_MODE %>" label=""  value="<%=HtmlUtil.escape(ermInformation.getRectificationRequestMode()) %>"></aui:input>
...
b)	Grievance Text raised by Subscriber :<div style=" width: 60rem; display: inline-block;"><aui:input type="textarea" name="<%=ErmFieldName.GRIEVANCE_TEXT %>"  label="" value="<%=HtmlUtil.escape(ermInformation.getGrievanceText()) %>">
 <aui:validator name="required" />
</aui:input></div>
...
$("#<portlet:namespace /><%=ErmFieldName.TIME_TYPE%>").change(function(){
	var x = document.getElementById("timeTypeDIV");
	var time_type=$("#<portlet:namespace /><%=ErmFieldName.TIME_TYPE%>").val();
